...
pub(super) fn compile(module: &mut Module<'_>, adapter: &AdapterData) {
    ...
    // When recursive reentrance is allowed and the lift/lower instances are
    // the same, force the sync-to-sync path regardless of async flags. This
    // avoids the concurrent runtime's cooperative scheduling which deadlocks
    // on same-instance async calls. The callee's core function is called
    // directly, bypassing task management entirely.
    let force_sync = module.allow_recursive_reentrance
    ...
    match (adapter.lower.options.async_, adapter.lift.options.async_) {
        (false, false) => {
            // Sync->sync case with only minimal use of intrinsics.
            let (compiler, lower_sig, lift_sig) = compiler(module, adapter);
            compiler.compile_sync_to_sync_adapter(adapter, &lower_sig, &lift_sig)
        }
        (true, true) => {
...
impl<'a, 'b> Compiler<'a, 'b> {
    ...
    fn compile_sync_to_sync_adapter(
        ...
        if adapter.called_as_export && !self.module.allow_recursive_reentrance {
            self.trap_if_not_flag(
                adapter.lift.flags,
                FLAG_MAY_ENTER,
        ...
        } else if self.module.debug && !self.module.allow_recursive_reentrance {
