...
impl Config {
    ...
    ///
    /// When enabled, the FACT compiler and the component inliner will not trap
    /// on adapters where the lift and lower instances are the same. Needed for
    /// fused components from tools like meld where multiple component instances
    /// are merged into one.
    #[cfg(feature = "component-model-async")]
